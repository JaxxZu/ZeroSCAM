...
class SpeechRecognizerUtil(private val context: Context, private val dao: TranscriptionDao) {
    ...
    // NEW: Connection State for Yating
    val yatingState = mutableStateOf(YatingState.IDLE)
    val yatingStatusMessage = mutableStateOf<String?>(null)
    ...
    fun validateYatingApiKey(apiKey: String) {
        if (apiKey.isBlank()) {
            yatingState.value = YatingState.ERROR
            yatingStatusMessage.value = "請輸入 API Key"
            return
        }

        yatingState.value = YatingState.VALIDATING
        yatingStatusMessage.value = "驗證中..."

        CoroutineScope(Dispatchers.IO).launch {
            val token = fetchYatingToken(apiKey)
            withContext(Dispatchers.Main) {
                if (token != null) {
                    yatingState.value = YatingState.CONNECTED
                    yatingStatusMessage.value = "連線成功"
                } else {
                    yatingState.value = YatingState.ERROR
                    yatingStatusMessage.value = "驗證失敗：無效的 API Key"
                }
            }
        }
    }

    fun resetYatingState() {
        yatingState.value = YatingState.IDLE
        yatingStatusMessage.value = null
    }
    ...
    private fun startYatingListening() {
        ...
        // Double check just in case, though UI should block it
        if (yatingState.value != YatingState.CONNECTED) {
            // Try one last quick validation attempt or fail
            if (apiKey.isBlank()) {
                errorState.value = "請先設定 Yating API Key"
                isRecording.value = false
                return
            }
        }
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // We fetch a fresh token for the actual session
                val token = fetchYatingToken(apiKey)

                if (token == null) {
                    CoroutineScope(Dispatchers.Main).launch {
                        errorState.value = "Yating 驗證失敗: 無法取得 Token"
                        yatingState.value = YatingState.ERROR // Downgrade state
                        stopRecording()
                    }
                    return@launch
                }
    ...
    fun startRecording() {
        ...
        // NEW: Check Yating connection state before allowing recording
        val provider = ServerConfigAsr.currentProvider
        if (provider.id == "yating") {
            if (yatingState.value != YatingState.CONNECTED) {
                errorState.value = "Yating 尚未連線，請檢查設定"
                return
            }
        }
